#!/usr/bin/env bash
# operation-m3.sh — Runner for Raven Operation M3 (Daily Verification).
...
LOG_BASENAME="operation-m3_${RUN_ID}"
RUNNER_LOG_FILE="$LOG_DIR/operation-m3_runner.ndjson"

mkdir -p "$LOG_DIR"

if [[ ! -f "$JAR_PATH" ]]; then
  printf '{"@timestamp":"%s","app":"raven-jobs","script":"operation-m3.sh","operation":"M3","run_id":"%s","event":"startup","status":"failed","error":"jar_not_found","jar_path":"%s"}\n' \
    "$(date -u +%Y-%m-%dT%H:%M:%SZ)" "$RUN_ID" "$JAR_PATH" >> "$RUNNER_LOG_FILE"
  echo "ERROR: JAR not found at $JAR_PATH" >&2
  exit 1
fi

printf '{"@timestamp":"%s","app":"raven-jobs","script":"operation-m3.sh","operation":"M3","run_id":"%s","event":"startup","status":"ok","jar_path":"%s","log_dir":"%s"}\n' \
...
printf '{"@timestamp":"%s","app":"raven-jobs","script":"operation-m3.sh","operation":"M3","run_id":"%s","event":"completion","status":"done","exit_code":%d,"app_log_file":"%s/%s.ndjson"}\n' \
...
if [[ "$EXIT_CODE" -eq 0 ]]; then
  ...
  if kdeconnect-cli --list-available 2>/dev/null | grep -q "$KDECONNECT_DEVICE"; then
    kdeconnect-cli -d "$KDECONNECT_DEVICE" --ping-msg \
      "raven-jobs | M3 | run_id=${RUN_ID} | event=completion | status=done | $(date -u +%Y-%m-%dT%H:%M:%SZ)" \
      2>/dev/null || true
  fi
fi
